<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Task') }} — {{ $task->project->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('tasks.update', [tenant()->slug, $task]) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="due_date" class="block mt-1 w-full" type="date" name="due_date" :value="old('due_date', optional($task->due_date)->format('Y-m-d'))" />
                        ...
                        <div class="mt-4">
                            ...
                            <select id="assigned_to" name="assigned_to" class="block mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                                ...
                                @foreach(tenant()->users as $member)
                                    <option value="{{ $member->id }}" {{ (string) old('assigned_to', $task->assigned_to) === (string) $member->id ? 'selected' : '' }}>{{ $member->name }} ({{ $member->email }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center justify-end gap-3 mt-6">
                            <a href="{{ route('projects.show', [tenant()->slug, $task->project]) }}" class="text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">{{ __('Cancel') }}</a>
                            <x-primary-button>
                                {{ __('Save Changes') }}
                            </x-primary-button>
                        </div>
